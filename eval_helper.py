# Copyright (c) 2023 Yuzhe Yang
# Licensed under the MIT License. See https://github.com/YyzHarry/SubpopBench for details.
import torch 
import numpy as np
import pandas as pd
...
def eval_metrics(algorithm, loader, device, thres=0.5):
    ...
    for y in np.unique(targets):
        ...
        res['per_class'][int(y)]['accuracy'] = accuracy_score(targets, preds_rounded)
    res['min_class_accuracy'] = min([res['per_class'][c]['accuracy'] for c in np.unique(targets)])
