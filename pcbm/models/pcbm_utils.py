...
class PosthocLinearCBM(nn.Module):
    ...
    def get_all_nonzero_concepts(self):
        weights = self.classifier.weight.clone().detach()

        spurious_dict = {self.idx_to_class.get(i, f"class_{i}"): [] for i in range(weights.size(0))}
        core_dict = {self.idx_to_class.get(i, f"class_{i}"): [] for i in range(weights.size(0))}

        spurious_start_idx = weights.size(1) - 221

        for concept_idx in range(0, spurious_start_idx):
            concept_weights = weights[:, concept_idx]  # shape: [num_classes]
            max_val, max_class_idx = torch.max(concept_weights, dim=0)

            if max_val > 0:
                cls_name = self.idx_to_class.get(max_class_idx.item(), f"class_{max_class_idx.item()}")
                concept_name = self.names[concept_idx]
                core_dict[cls_name].append(concept_name)

        for concept_idx in range(spurious_start_idx, weights.size(1)):
            ...
            if max_val > 0:
                cls_name = self.idx_to_class.get(max_class_idx.item(), f"class_{max_class_idx.item()}")
                concept_name = self.names[concept_idx]
                spurious_dict[cls_name].append(concept_name)

        return spurious_dict, core_dict


        # for class_idx in range(weights.size(0)):
        #     cls_name = self.idx_to_class.get(class_idx, f"class_{class_idx}")
        #     cls_weights = weights[class_idx][-221:]

        #     nonzero_indices = (cls_weights > 0).nonzero(as_tuple=True)[0].tolist()
        #     class_concepts = [self.names[i] for i in nonzero_indices]

        #     concept_dict[cls_name] = class_concepts
