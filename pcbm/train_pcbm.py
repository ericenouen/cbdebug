...
def run_linear_probe(args, train_data, test_data):
    ...
    if test_labels.max() == 1:
        ...
        coef = torch.tensor(classifier.coef_)
        intercept = torch.tensor(classifier.intercept_)
    
    sparsity = (coef == 0).float().mean()
    print(f"Sparsity coefficient: {sparsity:.2f}")
...
def main(args, backbone, preprocess):
    ...
    concepts_per_class = posthoc_layer.get_all_nonzero_concepts()
    save_path = f"concepts/{args.dataset}_{args.seed}_concepts.json"
    with open(save_path, 'w') as f:
        json.dump(concepts_per_class, f, indent=2)
