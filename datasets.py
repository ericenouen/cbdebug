...
class Waterbirds(SubpopDataset):
    ...
    def __init__(self, split, hparams, train_attr='yes', subsample_type=None, duplicates=None, transform=None):
        root = os.path.join(PATH_TO_DATA, "waterbirds", "waterbird_complete95_forest2water2")
        metadata = os.path.join(PATH_TO_DATA, "waterbirds", "metadata_waterbirds.csv")
...
class CelebA(SubpopDataset):
    ...
    def __init__(self, split, hparams, train_attr='yes', subsample_type=None, duplicates=None, transform=None):
        root = os.path.join(PATH_TO_DATA, "celeba", "img_align_celeba")
        metadata = os.path.join(PATH_TO_DATA, "celeba", "metadata_celeba.csv")
        if transform is None:
            transform = transforms.Compose([
...
class MetaShift(SubpopDataset):
    ...
    def __init__(self, split, hparams, train_attr='yes', subsample_type=None, duplicates=None, transform=None):
        # root = os.path.join(PATH_TO_DATA, "MetaShift", "MetaShift-subpopulation-shift")
        # metadata = os.path.join(PATH_TO_DATA, "MetaShift", "MetaShift-subpopulation-shift", "metadata_metashift.csv")
        root = os.path.join(PATH_TO_DATA, "MetaShiftCoco")
        metadata = os.path.join(PATH_TO_DATA, "MetaShiftCoco", "metadata_metashift.csv")
...
class Living17(BREEDSBase):
    def __init__(self, split, hparams, train_attr='no', subsample_type=None, duplicates=None):
        metadata = os.path.join(PATH_TO_DATA, "breeds", "metadata_living17.csv")
        super().__init__(metadata, split, train_attr, subsample_type, duplicates)
