...
@torch.no_grad() 
def get_concepts(net, concept_indices, dataloader):
    ...
    for i, (xs1, xs2, ys, _) in enumerate(tqdm(dataloader)):
        # if len(xl) == 2:
        #     xs = xl[0]
        # else:
        #     xs = xl
    ...
    for i, (xs1, xs2, ys, _) in enumerate(tqdm(dataloader)):
        for p in concept_indices:
            if i in [t[0] for t in topks[p]]:
                ...
                patchsize = 32
                skip = round((image_size - patchsize) / (pfs.shape[-1] - 1))
                h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(image_size, pfs.shape, patchsize, skip, h_idx, w_idx)
                img_tensor_patch = xs[0, :, h_coor_min:h_coor_max, w_coor_min:w_coor_max].cpu().detach()

                concept_list[p].append(img_tensor_patch)
