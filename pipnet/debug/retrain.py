...
def train_pipnet(algorithm, net, train_loader, optimizer_net, optimizer_classifier, scheduler_net, scheduler_classifier, criterion, epoch, nr_epochs, device, pretrain=False, finetune=False, progress_prefix: str = 'Train Epoch'):
    ...
    for i, (xs1, xs2, ys, idx) in train_iter:
        ...
        if algorithm.name == "Augment" or algorithm.name == "CBDebug":
            x = torch.cat([xs1, xs2])
            y = torch.cat([ys, ys])
            aug_x = algorithm.augment(x, y, idx)
            proto_features, pooled, out = net(aug_x)
        else:
            proto_features, pooled, out = net(torch.cat([xs1, xs2]))

        loss, acc = calculate_loss(algorithm, proto_features, pooled, out, ys, idx, align_pf_weight, t_weight, unif_weight, cl_weight, net.module._classification.normalization_multiplier, pretrain, finetune, criterion, train_iter, print=True, EPS=1e-8)

        if algorithm.name == "ProtoPDebug":
            loss += algorithm.compute_loss(net)
...
def calculate_loss(algorithm, proto_features, pooled, out, ys1, idx, align_pf_weight, t_weight, unif_weight, cl_weight, net_normalization_multiplier, pretrain, finetune, criterion, train_iter, print=True, EPS=1e-10):
    ...
    if not pretrain:
        if algorithm.name == "PermutationWeighting" or algorithm.name == "CBDebug":
            class_loss = algorithm.compute_loss(out, net_normalization_multiplier, ys, idx)
        else:
